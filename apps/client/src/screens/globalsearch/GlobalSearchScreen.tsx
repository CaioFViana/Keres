...
const GlobalSearchScreen = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    globalSearchService.searchAllEntities(storyId, trimmed, userId)
      .then((found) => {
  ...
  const sections = useMemo<ResultSection[]>(() => {
    ...
    const filteredResults = favoriteFilterState === 'all'
      ? results
      : results.filter((result) => result.isFavorite !== null && (
  ...
  const handleFavoriteFilterToggle = useCallback(() => {
    setFavoriteFilterState((current) => current === 'all'
      ? 'favorite'
      : current === 'favorite' ? 'not-favorite' : 'all');
  }, []);

  const favoriteFilterIcon: keyof typeof Ionicons.glyphMap = favoriteFilterState === 'favorite'
    ? 'star'
    : favoriteFilterState === 'not-favorite' ? 'ban-outline' : 'star-outline';
  const favoriteFilterColor = favoriteFilterState === 'favorite'
    ? colors.accent
    : favoriteFilterState === 'not-favorite' ? colors.notification : colors.primary;
