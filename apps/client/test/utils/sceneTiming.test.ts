...
const t = ((key: string, options?: { count?: number }) => `${key}:${options?.count ?? 0}`) as unknown as TFunction;

describe('scene timing', () => {
  it('uses zero minutes for incomplete values and surfaces valid durations', () => {
    expect(formatSceneUniverseDuration({ duration: null, durationType: 'hours' }, t)).toBe('scene_time_minutes:0');
  ...
  it('normalizes time using the narrative conversion rules', () => {
    expect(formatSceneUniverseDuration({ duration: 25, durationType: 'hours' }, t, true))
      .toBe('scene_time_days:1, scene_time_hours:1');
  });
