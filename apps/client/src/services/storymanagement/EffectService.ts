...
export interface EffectService {
  ...
  getEffectsByEntity(storyId: string, entityType: Effect['entityType'], entityId: string): Promise<Effect[]>;
  createEffect(userId: string, effectData: Omit<Effect, 'id' | 'createdAt' | 'updatedAt' | 'version' | 'isDeleted' | 'deletedAt'>): Promise<Effect>;
  updateEffect(userId: string, id: string, effectData: Partial<Omit<Effect, 'id' | 'storyId' | 'entityType' | 'entityId' | 'createdAt' | 'updatedAt' | 'version' | 'isDeleted' | 'deletedAt'>>): Promise<Effect>;
...
export const createEffectService = (db: AppDrizzleClient): EffectService => {
  ...
  return {
    ...
    async getEffectsByEntity(storyId: string, entityType: Effect['entityType'], entityId: string): Promise<Effect[]> {
      return db.query.effects.findMany({
        where: and(
    ...
    async createEffect(
      userId: string,
      effectData: Omit<Effect, 'id' | 'createdAt' | 'updatedAt' | 'version' | 'isDeleted' | 'deletedAt'>
    ): Promise<Effect> {
      ...
      await recordLocalOperation(db, newEffect.storyId, userIdToLog, 'create', 'Effect', newEffect.id, newEffect);
    ...
    async updateEffect(
      userId: string,
      id: string,
      effectData: Partial<Omit<Effect, 'id' | 'storyId' | 'entityType' | 'entityId' | 'createdAt' | 'updatedAt' | 'version' | 'isDeleted' | 'deletedAt'>>
    ): Promise<Effect> {
      ...
      if (Object.keys(changes).length > 0) {
        const userIdToLog = await getUserIdForOperation(db, serverService, updatedEffect.storyId, userId);
        await recordLocalOperation(db, updatedEffect.storyId, userIdToLog, 'update', 'Effect', updatedEffect.id, changes);
      }
    ...
    async deleteEffect(userId: string, id: string): Promise<void> {
      ...
      await db
        .update(effects)
        .set({ isDeleted: true, deletedAt: new Date(), updatedAt: new Date(), version: sql`${effects.version} + 1` })
        .where(eq(effects.id, id))
        .run();

      const userIdToLog = await getUserIdForOperation(db, serverService, effectToDelete.storyId, userId);
      await recordLocalOperation(db, effectToDelete.storyId, userIdToLog, 'delete', 'Effect', id, { id });
    },
  };
};
