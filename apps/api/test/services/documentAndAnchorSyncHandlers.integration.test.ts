...
beforeEach(async () => {
  ...
  await db
    .insert(stories)
    .values({ id: storyId, userId, title: 'A Queda', type: 'linear' } as never);
...
const documentCases: Array<[string, () => any, Record<string, unknown>, Record<string, unknown>]> =
  [
    [
      ...
      { name: 'Pins', description: null, content: { nodes: [], edges: [] } },
      { name: 'Pins revised' },
    ],
    [
      'LocationMap',
...
describe('JSON-document sync handlers', () => {
  it.each(documentCases)(
    '%s creates, version-updates, and tombstones its whole document',
    async (entity, build, data, changes) => {
      const handler = build();
      const id = newId();
  ...
  it.each(documentCases)(
    '%s lets only one concurrent edit based on the same version land',
      ...
      const results = await Promise.allSettled([
        handler.update(
          ...
          storyId,
          {
            ...
            entity,
            id,
            changes: { name: 'First edit', version: 1 },
          },
          current,
        ),
        handler.update(
          ...
          storyId,
          {
            ...
            entity,
            id,
            changes: { name: 'Second edit', version: 1 },
          },
          current,
        ),
      ]);
