...
export const adminRecoveryRoutes = new Elysia()
  ...
  .get('/deleted', async ({ query, user, set }) => {
    ...
    detail: {
      ...
      description: 'Lists tombstoned rows across all sync entity types (or a single one via ?entityType=), optionally scoped to a story.',
      tags: ['Admin'],
      security: [{ bearerAuth: [] }],
    },
  })

  .post('/:entityType/:id/restore', async ({ params, user, set }) => {
    ...
    detail: {
      ...
      description: 'Clears isDeleted/deletedAt via the same mechanism the sync pipeline uses for restore, and logs the action as an operation attributed to the admin.',
      tags: ['Admin'],
      security: [{ bearerAuth: [] }],
    },
  })

  .get('/operation-log', async ({ query, user, set }) => {
    await requireAdmin(user);
