import { CreateStoryUpdate, DeleteStoryUpdate, UpdateStoryUpdate, CharacterScene } from '@keres/shared';
import { eq } from 'drizzle-orm';
import { AppDrizzleClient } from '../../db';
import * as schema from '../../db/schema';
import { ClientSyncEntityHandler } from './ClientSyncEntityHandler';

export class CharacterSceneClientSyncHandler implements ClientSyncEntityHandler {
  entityName: string = 'CharacterScene';
  ...
  private get db(): AppDrizzleClient {
    if (!this.dbInstance) {
      throw new Error('CharacterSceneClientSyncHandler: Drizzle client (db) not set.');
    }
  ...
  async applyCreate(entityId: string, update: CreateStoryUpdate): Promise<void> {
    ...
    const data = update.data as CharacterScene;

    await this.db.insert(schema.characterScenes).values({
      ...data,
      id: update.id,
    ...
    console.log(`Applied create for CharacterScene ${update.id}`);
  }
  ...
  async applyUpdate(entityId: string, update: UpdateStoryUpdate): Promise<void> {
    ...
    await this.db.update(schema.characterScenes)
      ...
      .where(eq(schema.characterScenes.id, update.id));
    console.log(`Applied update for CharacterScene ${update.id}`);
  }
  ...
  async applyDelete(entityId: string, update: DeleteStoryUpdate): Promise<void> {
    ...
    await this.db.update(schema.characterScenes)
      ...
      .where(eq(schema.characterScenes.id, update.id));
    console.log(`Applied delete for CharacterScene ${update.id}`);
  }

  async getById(id: string): Promise<CharacterScene | undefined> {
    const entity = await this.db.query.characterScenes.findFirst({
      where: eq(schema.characterScenes.id, id),
    });
    return entity;
  }
}
