...
describe.each(HANDLERS)('$name client sync handler', ({ name, build, table, labelColumn, data, change }) => {
  const withDb = () => {
    const handler = build();
  ...
  const rowsOf = () => database.db.select().from(table as never).all() as any[];

  it('refuses to work before a database is set', async () => {
    const handler = build();

    await expect(handler.applyCreate(STORY_ID, createUpdate(name, 'e-1', data('e-1')))).rejects.toThrow(
      /Drizzle client \(db\) not set/,
    );
  });
  ...
  it('refuses a create with no id instead of inserting a broken row', async () => {
    ...
    await handler.applyCreate(STORY_ID, { type: 'create', entity: name, data: data('e-1') } as CreateStoryUpdate);
...
describe('CharacterClientSyncHandler specifics', () => {
  it('writes the optional fields the server sent', async () => {
    ...
    const row = await database.db.query.characters.findFirst({ where: eq(schema.characters.id, 'char-1') });
