...
import { LocationCreateSchema, LocationUpdateSchema, LocationResponseSchema } from '@keres/shared';
...
export class LocationController {
  ...
  async getLocationsByStoryId(c: Context) {
    ...
    try {
      ...
      return c.json(locations.map(location => LocationResponseSchema.parse(location)), 200);
    } catch (error) {
      return c.json({ error: 'Internal Server Error' }, 500);
    }
  }
  ...
  async updateLocation(c: Context) {
    ...
    try {
      ...
      if (!updatedLocation) {
        return c.json({ error: 'Location not found or does not belong to the specified story' }, 404);
      }
