...
describe('admin recovery', () => {
  it('lists and restores a soft-deleted story through the audited recovery path', async () => {
    ...
    expect(deleted.data).toEqual(
      expect.arrayContaining([expect.objectContaining({ entityType: 'Story', id: storyId })]),
    );

    const restored = await request('POST', `/admin/api/recovery/Story/${storyId}/restore`, {
      token: admin.token,
    });
    ...
    expect(log.data.items).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ entityId: storyId, userId: admin.userId }),
      ]),
    );
  });

  it('returns clear client errors for invalid recovery requests', async () => {
    const unknown = await request('POST', `/admin/api/recovery/Unknown/${newId()}/restore`, {
      token: admin.token,
    });
    const missing = await request('POST', `/admin/api/recovery/Story/${newId()}/restore`, {
      token: admin.token,
    });
    const malformed = await request('GET', '/admin/api/recovery/operation-log', {
      token: admin.token,
      query: { page: 0 },
    });
