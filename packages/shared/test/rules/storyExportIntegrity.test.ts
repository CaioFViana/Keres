...
describe("story export integrity", () => {
  it("prunes required dangling references until cascaded rows are stable", () => {
    const pruned = pruneDanglingStoryExportRows(
      buildExport({
        ...
        choiceCheckGroups: [
          { id: "group-dead", storyId: "story-1", choiceId: "choice-dead" },
        ],
        choiceChecks: [
          { id: "check-dead", storyId: "story-1", groupId: "group-dead" },
        ],
      }),
    );
    expect(pruned.characterScenes).toEqual([
      expect.objectContaining({ id: "presence-live" }),
    ]);
  ...
  it("refuses a reference to an entity the file does not carry", () => {
    const violations = findStoryExportIntegrityErrors(
      buildExport({
        characterRelations: [
          relation("rel-1", "char-a", "char-missing", "Trust"),
        ],
      }),
    );
  ...
  it("reports a duplicate with no constraint behind it as a warning, not an error", () => {
    ...
    expect(findStoryExportIntegrityViolations(story)[0].severity).toBe(
      "warning",
    );
  ...
  it("throws only on errors, carrying them on the exception", () => {
    ...
    expect(() => assertStoryExportIntegrity(corrupt)).toThrow(
      StoryIntegrityError,
    );
    try {
      assertStoryExportIntegrity(corrupt);
    } catch (error) {
      expect((error as StoryIntegrityError).violations[0].ids).toEqual([
        "rel-2",
      ]);
    }
  });
