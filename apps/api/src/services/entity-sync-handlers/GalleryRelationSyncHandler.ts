import { CreateGalleryRelationDataSchema, CreateGalleryRelationDataType, CreateStoryUpdate, GalleryOwnerEntity, PartialGalleryRelationSchema, UpdateStoryUpdate } from '@keres/shared';
 ...
 */
export class GalleryRelationSyncHandler extends BaseSyncEntityHandler<typeof CreateGalleryRelationDataSchema, typeof PartialGalleryRelationSchema> {
  ...
  private async validateRelation(storyId: string, galleryId: string, ownerId: string, ownerType: GalleryOwnerEntity): Promise<void> {
  ...
  private async findActiveDuplicate(storyId: string, galleryId: string, ownerId: string, ownerType: string) {
  ...
  async create(userId: string, storyId: string, update: CreateStoryUpdate): Promise<void> {
    ...
    await this.validateRelation(storyId, validatedData.galleryId, validatedData.ownerId, validatedData.ownerType);

    const duplicate = await this.findActiveDuplicate(storyId, validatedData.galleryId, validatedData.ownerId, validatedData.ownerType);
    if (duplicate) {
      throw new Error(`Conflict: Gallery ${validatedData.galleryId} is already linked to ${validatedData.ownerType} ${validatedData.ownerId} in story ${storyId}.`);
    }
  ...
  async update(userId: string, storyId: string, update: UpdateStoryUpdate, currentEntity: any): Promise<void> {
    ...
    if (retargeted) {
      ...
      const duplicate = await this.findActiveDuplicate(storyId, newGalleryId, newOwnerId, newOwnerType);
      if (duplicate && duplicate.id !== update.id) {
        throw new Error(`Conflict: Gallery ${newGalleryId} is already linked to ${newOwnerType} ${newOwnerId} in story ${storyId}.`);
      }
    }
