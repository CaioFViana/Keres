...
import { CharacterSelect, ChapterSelect, ChoiceSelect, ItemJourneySelect, ItemSelect, SceneSelect } from '../../db/schema';
...
const ItemJourneyTimeline: React.FC<ItemJourneyTimelineProps> = ({ item, storyId, storyType }) => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [fetchedJourneys, fetchedScenes, fetchedChapters, fetchedChoices, fetchedCharacters] = await Promise.all([
  ...
  const orderedJourneys = orderItemJourneysByNarrative(journeys, storyType, scenes, choices, chapters);
  ...
  const handleAddJourney = useCallback(() => {
    const drawerNavigation = navigation.getParent<DrawerNavigationProp<MainSystemDrawerParamList>>();
    if (drawerNavigation) {
      drawerNavigation.navigate('ItemJourneysStack', { screen: 'ItemJourneyForm', params: { itemId: item.id } });
    }
  }, [navigation, item.id]);
  ...
  const renderChapterAccent = (chapterId: string | undefined) => (
    <View style={[styles.chapterAccent, { backgroundColor: chapterId ? chapterColorById.get(chapterId) ?? colors.border : colors.border, width: 40 }]} />
  );
      ...
      {loading ? (
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
          <View style={[styles.card, styles.originCard]}>
            ...
            <Text style={styles.cardTitle} numberOfLines={2}>{item.initialState || t('common_na')}</Text>
            {originOwner && <Text style={styles.cardSubtitle} numberOfLines={1}>{originOwner.name}</Text>}
          </View>
          ...
          {orderedJourneys.map((journey) => {
            ...
            const newOwner = journey.newCharacterOwnerId ? characterById.get(journey.newCharacterOwnerId) : undefined;
            return (
              <View key={journey.id} style={styles.row}>
                <Ionicons name="arrow-forward" size={16} color={colors.textSecondary} style={styles.connector} />
                <TouchableOpacity style={styles.card} onPress={() => handleOpenJourney(journey.id)} activeOpacity={0.7}>
                  {renderChapterAccent(chapter?.id)}
                  <Text style={styles.cardLabel} numberOfLines={1}>{scene?.name ?? t('unknown_scene')}</Text>
                  <Text style={styles.cardTitle} numberOfLines={2}>{journey.newState}</Text>
                  {newOwner && <Text style={styles.cardSubtitle} numberOfLines={1}>{newOwner.name}</Text>}
                </TouchableOpacity>
              </View>
            );
          })}
          ...
          <View style={styles.row}>
            <Ionicons name="arrow-forward" size={16} color={colors.textSecondary} style={styles.connector} />
            <TouchableOpacity style={[styles.card, styles.addCard]} onPress={handleAddJourney} activeOpacity={0.7}>
              <Ionicons name="add-circle-outline" size={22} color={colors.primary} />
              <Text style={[styles.cardSubtitle, { marginTop: 4, color: colors.primary }]}>{t('add_item_journey')}</Text>
            </TouchableOpacity>
          </View>
        </ScrollView>
      )}
