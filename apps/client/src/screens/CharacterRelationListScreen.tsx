...
import { getCharacterRelationsByCharacterId, createCharacterRelation, updateCharacterRelation, deleteCharacterRelation } from '../api';
import { CharacterRelationResponse, CreateCharacterRelationPayload, UpdateCharacterRelationPayload } from '@keres/shared';
...
export default function CharacterRelationListScreen({ token, characterId, navigation }: CharacterRelationListScreenProps) {
  ...
  const [editingCharacterRelation, setEditingCharacterRelation] = useState<CharacterRelationResponse | null>(null);
  ...
  const fetchCharacterRelations = async () => {
    try {
      ...
      Alert.alert('Error fetching character relations', error.response?.data?.error || 'Something went wrong.');
    }
  };
  ...
  const handleCreateCharacterRelation = async () => {
    ...
    try {
      ...
      Alert.alert('Error creating character relation', error.response?.data?.error || 'Something went wrong.');
    }
  };
  ...
  const handleUpdateCharacterRelation = async () => {
    ...
    try {
      ...
      Alert.alert('Error updating character relation', error.response?.data?.error || 'Something went wrong.');
    }
  };
        ...
        { 
          ...
          onPress: async () => {
            try {
              ...
              Alert.alert('Error deleting character relation', error.response?.data?.error || 'Something went wrong.');
            }
          },
        },
      ],
  ...
  const renderCharacterRelationItem = ({ item }: { item: CharacterRelationResponse }) => (
    <View style={styles.characterRelationItem}>
      ...
      <View style={styles.characterRelationActions}>
        ...
        <Button mode="outlined" onPress={() => handleDeleteCharacterRelation(item.id)} buttonColor="red">
          Delete
        </Button>
      </View>
    </View>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.form}>
        <TextInput
          label={editingCharacterRelation ? 'Edit Character Relation Description' : 'New Character Relation Description'}
        ...
        <Button
          mode="contained"
          onPress={editingCharacterRelation ? handleUpdateCharacterRelation : handleCreateCharacterRelation}
          style={styles.button}
        >
