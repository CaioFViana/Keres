import type { Choice, CreateStoryUpdate, DeleteStoryUpdate, UpdateStoryUpdate } from '@keres/shared';
import { eq } from 'drizzle-orm';
import type { AppDrizzleClient } from '../../db';
import * as schema from '../../db/schema';
import type { ClientSyncEntityHandler } from './ClientSyncEntityHandler';

export class ChoiceClientSyncHandler implements ClientSyncEntityHandler {
  entityName: string = 'Choice';
  ...
  private get db(): AppDrizzleClient {
    if (!this.dbInstance) {
      throw new Error('ChoiceClientSyncHandler: Drizzle client (db) not set.');
    }
  ...
  async applyCreate(entityId: string, update: CreateStoryUpdate): Promise<void> {
    ...
    const data = update.data as Choice;

    await this.db.insert(schema.choices).values({
      ...data,
      id: update.id,
    ...
    console.log(`Applied create for Choice ${update.id}`);
  }
  ...
  async applyUpdate(entityId: string, update: UpdateStoryUpdate): Promise<void> {
    ...
    await this.db
      ...
      .where(eq(schema.choices.id, update.id));
    console.log(`Applied update for Choice ${update.id}`);
  }
  ...
  async applyDelete(entityId: string, update: DeleteStoryUpdate): Promise<void> {
    ...
    await this.db
      ...
      .where(eq(schema.choices.id, update.id));
    console.log(`Applied delete for Choice ${update.id}`);
  }

  async getById(id: string): Promise<Choice | undefined> {
    const entity = await this.db.query.choices.findFirst({
      where: eq(schema.choices.id, id),
    });
    return entity;
  }
}
