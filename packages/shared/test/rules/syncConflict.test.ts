...
describe("sync conflict rules", () => {
  it("compares primitives, nulls, dates, and structured values consistently", () => {
    ...
    expect(
      syncConflictValuesDiffer(new Date("2025-01-01"), new Date("2025-01-01")),
    ).toBe(false);
    expect(
      syncConflictValuesDiffer(new Date("2025-01-01"), new Date("2025-01-02")),
    ).toBe(true);
  ...
  it("finds only actual content conflicts and ignores synchronization bookkeeping", () => {
    expect(
      findContestedFields({ id: "x", title: "Local", count: 1 }, null),
    ).toEqual(["title", "count"]);
    expect(
      findContestedFields(
