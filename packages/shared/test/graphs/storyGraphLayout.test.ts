...
describe("wrapLabel", () => {
  ...
  it("truncates labels that exceed the configured line budget", () => {
    expect(wrapLabel("one two three four five", 7, 2)).toEqual([
      "one two",
      "three…",
    ]);
...
describe("buildStoryGraphLayout", () => {
  ...
  it("lays out all edge kinds, dangling choices, and detached scenes", () => {
    ...
    expect(
      layout.nodes.find((node) => node.id === "detached")?.isDetached,
    ).toBe(true);
    expect(
      layout.edges.every(
        (edge) => edge.path.startsWith("M ") && edge.arrowPoints.length > 0,
      ),
    ).toBe(true);
  });
