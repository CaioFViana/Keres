...
describe('buildStoryAnalysisReport', () => {
  ...
  describe('choice checks', () => {
    ...
    it('cascades through two levels: a choice gated on a scene that is itself only reachable via a never-satisfiable choice', () => {
      ...
      input.choices.push({ id: 'toSceneX', sceneId: 'start', nextSceneId: 'sceneX', text: 'go to X' });
      input.choiceCheckGroups.push({ id: 'gX', choiceId: 'toSceneX', combinator: 'AND' });
      input.choiceChecks.push({
        id: 'cX', groupId: 'gX', mode: 'enable', type: 'inventory',
        sceneId: null, minVisits: null, itemId: 'item', itemPresence: 'has', triggerName: null, triggerState: null,
      });
      ...
      input.choices.push({ id: 'toSceneY', sceneId: 'start', nextSceneId: 'sceneY', text: 'go to Y' });
      input.choiceCheckGroups.push({ id: 'gY', choiceId: 'toSceneY', combinator: 'AND' });
      input.choiceChecks.push({
        id: 'cY', groupId: 'gY', mode: 'enable', type: 'sceneCount',
        sceneId: 'sceneX', minVisits: 1, itemId: null, itemPresence: null, triggerName: null, triggerState: null,
      });
