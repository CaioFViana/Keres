...
import { boardNodeSize } from '../../src/utils/boardLayout';
...
it('meets the node on its rectangle, not at the centre', () => {
  ...
  const size = boardNodeSize(to);
  const centre = { x: to.x + size.width / 2, y: to.y + size.height / 2 };
  const border = nodeBorderPoint(to, from.x + size.width / 2, from.y + size.height / 2);
