...
import { db, characterMoments } from '@keres/db'; // Import db and characterMoments table
...
export class CharacterMomentRepository implements ICharacterMomentRepository {
  ...
  async findByCharacterId(characterId: string): Promise<CharacterMoment[]> {
    ...
    try {
      const results = await db.select().from(characterMoments).where(eq(characterMoments.characterId, characterId));
  ...
  async findByMomentId(momentId: string): Promise<CharacterMoment[]> {
    ...
    try {
      const results = await db.select().from(characterMoments).where(eq(characterMoments.momentId, momentId));
      return results.map(this.toDomain);
    } catch (error) {
  ...
  async delete(characterId: string, momentId: string): Promise<void> {
    ...
    try {
      await db.delete(characterMoments).where(and(eq(characterMoments.characterId, characterId), eq(characterMoments.momentId, momentId)));
    } catch (error) {
  ...
  private toPersistence(characterMomentData: CharacterMoment): typeof characterMoments.$inferInsert {
