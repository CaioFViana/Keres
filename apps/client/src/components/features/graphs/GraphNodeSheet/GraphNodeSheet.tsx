...
const GraphNodeSheet: React.FC<GraphNodeSheetProps> = ({
  ...
  return (
    <ResponsiveModal visible onClose={onClose} placement="adaptive" contentStyle={styles.sheet} maxHeight="78%">
      <View style={styles.handle} />
      <View style={styles.header}>
        <View style={styles.headerText}>
          <Text style={styles.title}>{title}</Text>
          {subtitle && <Text style={[styles.subtitle, { color: subtitle.color || colors.textSecondary }]}>{subtitle.text}</Text>}
        </View>
            ...
            {section.items && (section.items.length === 0
              ...
              : section.items.map(item => (
                <TouchableOpacity key={item.id} style={styles.item} onPress={item.onPress}>
                  <Ionicons name={item.icon} size={16} color={colors.textSecondary} style={{ marginRight: 9 }} />
