import { AdminCreateUserSchema, AdminUpdateUserSchema, AdminUserListQuerySchema } from '@keres/shared';
...
export const adminUserRoutes = new Elysia()
  ...
  .get('/', async ({ query, user, set }) => {
    ...
    detail: { summary: 'List users (paginated, filterable)', tags: ['Admin'], security: [{ bearerAuth: [] }] },
  })
  ...
  .put('/:id', async ({ params, body, user, set }) => {
    ...
    detail: { summary: 'Update a user (profile, isAdmin, tierId)', tags: ['Admin'], security: [{ bearerAuth: [] }] },
  })
