...
import { GalleryCreateSchema, GalleryUpdateSchema, GalleryResponseSchema } from '@keres/shared';
...
export class GalleryController {
  ...
  async getGalleryByOwnerId(c: Context) {
    ...
    try {
      ...
      return c.json(galleryItems.map(item => GalleryResponseSchema.parse(item)), 200);
    } catch (error) {
      return c.json({ error: 'Internal Server Error' }, 500);
    }
  }
  ...
  async updateGallery(c: Context) {
    ...
    try {
      ...
      if (!updatedGallery) {
        return c.json({ error: 'Gallery item not found or does not belong to the specified story/owner' }, 404);
      }
