...
import { Item } from '@keres/shared/entities/Item';
import { eq } from 'drizzle-orm';
import { AppDrizzleClient } from '../../db';
import * as schema from '../../db/schema';
import { ClientSyncEntityHandler } from './ClientSyncEntityHandler';

export class ItemClientSyncHandler implements ClientSyncEntityHandler {
  ...
  private get db(): AppDrizzleClient {
    if (!this.dbInstance) {
      throw new Error('ItemClientSyncHandler: Drizzle client (db) not set.');
    }
  ...
  async applyCreate(entityId: string, update: CreateStoryUpdate): Promise<void> {
    ...
    const data = update.data as Item;
  ...
  async applyUpdate(entityId: string, update: UpdateStoryUpdate): Promise<void> {
    ...
    await this.db
      ...
      .where(eq(schema.items.id, update.id));
    console.log(`Applied update for Item ${update.id}`);
  }
  ...
  async applyDelete(entityId: string, update: DeleteStoryUpdate): Promise<void> {
    ...
    await this.db
      ...
      .where(eq(schema.items.id, update.id));
    console.log(`Applied delete for Item ${update.id}`);
  }

  async getById(id: string): Promise<Item | undefined> {
    const entity = await this.db.query.items.findFirst({
