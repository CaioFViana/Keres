...
const ENTITY_LABELS: Record<string, string> = {
  Character: 'characters_title', CharacterRelation: 'character_relations_title', Item: 'items_title', ItemJourney: 'item_journeys_title',
};
const SCHEMA_ENTITY_LABELS: Record<StorySchemaEntityType, string> = {
  Character: 'characters_title', Location: 'locations_title', Item: 'items_title', Scene: 'scenes_title', Chapter: 'chapters_title', Note: 'notes_title', WorldRule: 'world_rules_title',
};
...
const SuggestionsScreen = () => {
  ...
  const loadGroups = useCallback(async () => {
    ...
    const native = Object.entries(entityFieldMetadata).flatMap(([entity, fields]) => fields
      .filter((field) => field.isSuggestion && field.suggestionsSource)
      .map((field) => ({ type: field.suggestionsSource!, key: field.suggestionsSource!, label: `${t(ENTITY_LABELS[entity] ?? entity)} · ${t(field.label)}` })));
    const schemaService = createStorySchemaFieldService(db);
    const batches = await Promise.all(STORY_SCHEMA_ENTITY_TYPES.map((entityType) => schemaService.getFieldsByStoryAndEntityType(storyId, entityType)));
    const custom = batches.flatMap((fields, index) => fields
      .filter((field) => field.type === AttributeType.SUGGESTION)
      .map((field) => ({ type: customAttributeSuggestionType(field.id), key: field.key, label: `${t(SCHEMA_ENTITY_LABELS[STORY_SCHEMA_ENTITY_TYPES[index]])} · ${field.name}` })));
    const next = [...native, ...custom].sort((a, b) => a.label.localeCompare(b.label));
    setGroups(next);
    setSelectedType((current) => current && next.some((group) => group.type === current) ? current : next[0]?.type ?? null);
  }, [db, storyId, t]);
  ...
  useEffect(() => {
    const refresh = (changedStoryId: string) => { if (changedStoryId === storyId) loadStored(); };
  ...
  const add = async () => {
    ...
    try { await suggestionService.createSuggestion(userId, selectedType, newValue, storyId); setNewValue(''); }
    catch (error) { AppAlert.alert(t('error'), (error as Error).message); }
  };
  const saveEdit = async () => {
    if (!userId || !editingId || !editingValue.trim()) return;
    try { await suggestionService.updateSuggestion(userId, editingId, editingValue); setEditingId(null); }
    catch (error) { AppAlert.alert(t('error'), (error as Error).message); }
  };
  const remove = (id: string) => AppAlert.alert(t('delete'), t('delete_suggestion_message'), [
    { text: t('cancel'), style: 'cancel' },
    { text: t('delete'), style: 'destructive', onPress: async () => { if (userId) await suggestionService.deleteSuggestion(userId, id); } },
  ]);
