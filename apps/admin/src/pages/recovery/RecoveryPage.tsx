...
const ENTITY_TYPES = [
  'Story', 'Character', 'Chapter', 'Scene', 'Location', 'Note', 'WorldRule', 'Choice',
  'Gallery', 'GalleryRelation', 'Item', 'ItemJourney', 'CharacterRelation', 'CharacterScene',
  'Tag', 'TagRelation', 'NoteRelation', 'Suggestion',
];
...
export function RecoveryPage() {
  ...
  const search = () => {
    setLoading(true);
    setError(null);
    RecoveryApiService.listDeleted({ entityType: entityType || undefined, storyId: storyId || undefined })
      .then(setItems)
  ...
  const restore = async (item: DeletedItem) => {
    ...
    try {
      ...
      setItems((prev) => prev.filter((i) => !(i.entityType === item.entityType && i.id === item.id)));
    } catch (err) {
    ...
    <div>
      ...
      <section>
        ...
        <div className="toolbar">
          ...
          <input placeholder="Story ID (optional)" value={storyId} onChange={(e) => setStoryId(e.target.value)} />
      ...
      <section>
        <h2>Operation log</h2>
        <div className="toolbar">
          <input placeholder="Story ID" value={logStoryId} onChange={(e) => setLogStoryId(e.target.value)} />
        ...
        <div className="log-layout">
          <table className="data-table">
            <thead>
              <tr><th>When</th><th>Type</th><th>Entity</th><th>User</th></tr>
            </thead>
            <tbody>
              ...
              {logEntries.length === 0 && <tr><td colSpan={4}>No entries. Search above.</td></tr>}
            </tbody>
          </table>
          ...
          {selectedEntry && (
            <div className="detail-panel">
              ...
              <dl>
                <dt>Type</dt><dd>{selectedEntry.operationType}</dd>
                <dt>Entity</dt><dd>{selectedEntry.entityType}:{selectedEntry.entityId}</dd>
                <dt>User</dt><dd>{selectedEntry.userId}</dd>
                <dt>Operation version</dt><dd>{selectedEntry.operationVersion}</dd>
                <dt>Entity version</dt><dd>{selectedEntry.entityVersion ?? '-'}</dd>
