...
/** The ionicons SVGs are drawn on a 512×512 viewBox. */
 ...
 * picks fall back to the solid name); `keres:` and unknown namespaces resolve to nothing
 * until the pack renderer exists, so the point renders as a coloured circle only.
 */
function resolveMapIconShapes(iconName: string): string {
  const key = (iconName ?? '').trim();
  if (!key) return '';
  const resolved = resolveMapIcon(key);
  if (resolved.family !== 'ion' || !resolved.glyph) return '';
