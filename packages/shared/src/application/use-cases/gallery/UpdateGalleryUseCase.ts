...
import type { GalleryResponse, GalleryUpdatePayload } from 'schemas'
...
export class UpdateGalleryUseCase {
  ...
  async execute(
    ...
    if (data.imagePath !== undefined || fileBuffer !== undefined) {
      ...
      await this.fileSystemService.createDirectory(galleryPath) // Ensure directory exists
      ...
      if (fileBuffer) {
        ...
        if (existingGallery.isFile) {
          ...
          try {
            await this.fileSystemService.deleteFile(oldFilePath)
          } catch (error) {
        ...
        await this.fileSystemService.writeFile(fullFilePath, fileBuffer)
