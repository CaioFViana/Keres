...
describe('admin login', () => {
  ...
  it('keeps the token only after confirming admin access', async () => {
    mocks.post.mockResolvedValue({ data: { accessToken: 'token', userId: 'user-1', username: 'admin' } });
    mocks.get.mockResolvedValue({ data: [] });

    await expect(login('admin', 'password')).resolves.toEqual({ userId: 'user-1', username: 'admin' });
  ...
  it('clears the token when the authenticated account is not an admin', async () => {
    mocks.post.mockResolvedValue({ data: { accessToken: 'token', userId: 'user-1', username: 'reader' } });
    mocks.get.mockRejectedValue(new Error('Forbidden'));

    await expect(login('reader', 'password')).rejects.toThrow('This account does not have admin access.');
