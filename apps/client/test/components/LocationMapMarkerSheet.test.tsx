...
jest.mock('../../src/theme', () => ({
  useTheme: () => ({ colors: { surface: '#111', text: '#fff', textSecondary: '#aaa', border: '#444', error: '#c33', primary: '#85f' } }),
}));
jest.mock('react-i18next', () => ({ useTranslation: () => ({ t: (key: string) => key }) }));
jest.mock('../../src/components/layout/ResponsiveModal/ResponsiveModal', () => ({ children }: { children: React.ReactNode }) => <>{children}</>);
jest.mock('../../src/components/common/controls/Button/Button', () => ({ children }: { children: React.ReactNode }) => <>{children}</>);
...
it('keeps inner padding around marker-sheet controls so input borders can render fully', async () => {
  ...
  expect(view.getByTestId('location-map-marker-sheet-scroll')).toHaveStyle({
    paddingHorizontal:
