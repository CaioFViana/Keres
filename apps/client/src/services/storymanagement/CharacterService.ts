...
export interface CharacterService {
  ...
  ): Promise<CharacterSelect>; // Add createCharacter
  updateCharacter(
    currentUserId: string,
    characterId: string,
    updatedFields: Partial<Omit<CharacterSelect, 'id' | 'createdAt' | 'updatedAt' | 'version'>>,
  ): Promise<CharacterSelect>; // Changed return type
  ...
  return {
      ...
      switch (sortBy) {
        case 'name':
          orderByConditions.push(
            sortDirection === 'desc' ? desc(characters.name) : asc(characters.name),
          );
          break;
        case 'createdAt':
          orderByConditions.push(
            sortDirection === 'desc' ? desc(characters.createdAt) : asc(characters.createdAt),
          );
          break;
        case 'updatedAt':
          orderByConditions.push(
            sortDirection === 'desc' ? desc(characters.updatedAt) : asc(characters.updatedAt),
          );
          break;
        default:
    ...
    async updateCharacter(
      ...
      if (Object.keys(changes).length === 0) {
        console.log(
          `No significant changes detected for character ${characterId}. Skipping update and operation log.`,
        );
    ...
    async deleteCharacter(currentUserId: string, characterId: string): Promise<void> {
      const characterToDelete = await db.query.characters.findFirst({
        where: eq(characters.id, characterId),
      });
