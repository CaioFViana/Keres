 ...
 * "go to page X" cross-references. Refreshes on every change that could move a section,
 * including saves from the inline editor (`scene_changed`).
 */
export function useManuscriptData(storyId: string | undefined | null) {
  ...
  return { chapters, scenes, routes, choices, stepsByRouteId, loading, reload };
}
