...
export const adminRegistrationRoutes = new Elysia()
  ...
  .get('/', async ({ user }) => {
    ...
    detail: { summary: 'Get registration settings (open/closed signup, max users, default tier)', tags: ['Admin'], security: [{ bearerAuth: [] }] },
  })
  ...
  .put('/', async ({ body, user, set }) => {
    ...
    detail: { summary: 'Update registration settings', tags: ['Admin'], security: [{ bearerAuth: [] }] },
  });
